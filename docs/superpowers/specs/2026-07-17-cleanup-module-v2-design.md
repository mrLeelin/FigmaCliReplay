...
V2 已作为插件“AI 整理节点”的当前实现：Codex 与 Claude Code 进入同一规划接口；审批只认已校验计划和快照哈希；Figma 写入由 `apply_cleanup_plan.py` 提交一个精确事务；旧 `localAiRunner` 不再保存整理阶段、计划、审批或 CLI 会话状态。

回滚不完整时，当前页面会发现隐藏、锁定的 `__cleanup_backup__*` 根备份。用户可以显式选择一个受损根节点后恢复、删除备份或暂不处理。恢复通过克隆备份替换受损根节点，因此根节点 ID 可能变化；备份本身不持久保存源节点 ID，并被 PSD 增量匹配、Unity 导出和后续整理快照统一排除。
...
插件只显示一个用户概念：`AI`。整理面板在“AI 整理”按钮旁显示一个 AI 下拉框，首批支持：
...
规划 Provider、确定性执行器和整理状态机只属于内部实现，不作为并列选项暴露给用户。整理流程固定为：

1. 用户选择目标节点、AI，并点击“AI 自动整理节点”。
2. 当前页面立即显示整理进度，不跳转到“AI 执行”页。
3. 计划通过校验后，打开同一个整理弹窗，展示全部精确操作、警告以及“确认整理/取消”按钮。
4. 用户确认后，弹窗切换为应用和验证进度；执行器在后台消费已确认计划。
5. 成功、自动回滚或需要恢复时，弹窗显示最终结果；用户无需理解 planning、provider、executor 等内部术语。

通用 AI 任务仍可保留自己的任务详情页，但整理流程不得要求用户在“AI 提示词”和“AI 执行”两个页面之间往返。界面中不得同时出现“执行器”和“整理规划 AI”两个选择框，也不得显示“整理 Provider”之类的内部文案。
