...
职责：
- 串联 Relay health、analyze、plan、apply、verify，减少人工重复命令。
...
from figma_hierarchy_cleanup_cli import (
    DEFAULT_RELAY_URL,
    DEFAULT_OUTPUT_DIR,
    assert_completed,
...
def submit_and_save_job(
    ...
    """提交 Relay 任务，先保存结果再检查状态，方便失败时排查。"""
...
def run_explicit_nested_targets(args: argparse.Namespace, relay_url: str, root_target: Dict[str, str], work_dir: Path) -> Tuple[int, Dict[str, Any]]:
    ...
    health = run_timed_step(timings, "health", lambda: ensure_relay(relay_url, startup_timeout=args.startup_timeout))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--relay-url", default=DEFAULT_RELAY_URL, help="本地 Relay 地址")
    ...
    parser.add_argument("--job-name", default="Figma_Hierarchy_Cleanup_Pipeline", help="Relay job 名称")
