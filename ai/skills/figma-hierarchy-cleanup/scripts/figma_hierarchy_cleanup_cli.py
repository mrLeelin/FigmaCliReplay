...
from figma_relay_cli import (  # noqa: E402
    ...
    query_node_children as relay_query_node_children,
...
def preflight_target_probe(relay_url: str, node_id: str = "") -> Optional[Dict[str, Any]]:
    """Probe the live CLI path when health session metadata is stale or empty."""
    global ACTIVE_PREFLIGHT_PROBE
    if not ACTIVE_FILE_KEY and not ACTIVE_SESSION_ID and not node_id:
        return None
    try:
        if node_id:
            payload = relay_query_node_children(
                node_id,
...
def ensure_relay(relay_url: str, startup_timeout: float = 10.0, node_id: str = "") -> Dict[str, Any]:
    """确保 figmaRelay 本地 companion 可用，并返回 health 信息。"""
    global ACTIVE_SESSION_ID, ACTIVE_SESSION_REFRESH
    del startup_timeout
    health_payload = relay_health(relay_url=relay_url.rstrip("/"))
...
def submit_job(relay_url: str, job: Dict[str, Any], timeout: float, interval: float) -> Dict[str, Any]:
    """提交 Relay job 并等待 Figma 插件执行完成。"""
    del interval
    request_id = str(uuid.uuid4())
    job = attach_target_fields(job)
    return relay_submit_job(
        job,
        {},
        relay_url=relay_url.rstrip("/"),
...
def assert_completed(result_payload: Dict[str, Any]) -> None:
    """检查 Relay result 是否完成且没有阻塞错误。"""
    result = result_of_payload(result_payload)
    if not isinstance(result, dict):
        raise McpRelayError("Relay 返回缺少 result 对象")
    if result.get("status") != "completed":
...
def result_of_payload(result_payload: Dict[str, Any]) -> Dict[str, Any]:
    """Return the innermost Figma plugin result from CLI wrapper payloads."""
...
def query_node_children(relay_url: str, node_id: str, timeout: float = 15.0, interval: float = 0.3) -> Dict[str, Any]:
    """通过 Relay 查询节点直接子节点。"""
    del interval
    return relay_query_node_children(
        node_id,
        relay_url=relay_url.rstrip("/"),
        timeout=timeout,
        file_key=ACTIVE_FILE_KEY,
...
def run_analyze(args: argparse.Namespace) -> int:
    ...
    ensure_relay(relay_url, startup_timeout=args.startup_timeout, node_id=target.get("nodeId", ""))
...
def run_move_nodes(args: argparse.Namespace) -> int:
    ...
    ensure_relay(relay_url, startup_timeout=args.startup_timeout, node_id=source_node_id)
    job = {
...
def parse_args() -> argparse.Namespace:
    """解析命令行参数。"""
    parser = argparse.ArgumentParser(description="Figma 层级整理 Relay 客户端")
    ...
    parser.add_argument("--startup-timeout", type=float, default=10.0, help="等待 Relay 启动秒数")
    ...
    parser.add_argument("--job-name", default="Figma_Hierarchy_Cleanup", help="Relay job 名称")
