...
"""Prefab To Figma 专用 CLI 命令行入口。

职责：
- 通过 figmaRelay 连接已启动的本地 companion。
...
- Figma Desktop 必须打开目标文件，并运行 figma-relay 插件。
"""
...
DEFAULT_RESULT_PATH = Path(".tmp/prefab-to-figma/Panel/prefab_to_figma_relay_result.json")
...
from figma_relay_cli import health as relay_health, submit_job as relay_submit_job  # noqa: E402
...
def ensure_relay(relay_url: str, startup_timeout: float = 10.0) -> dict[str, Any]:
    """确保 figmaRelay 本地 companion 可用，并返回 health 信息。"""
...
def build_job(
    ...
    """根据导出包和写入计划构建 Relay job 与 assetPaths。"""

    package = load_json(package_path)
    write_plan = load_json(write_plan_path)

    # 移除 missingSprite 节点的 image 数据，避免 Relay 尝试上传不存在的图片
...
def build_asset_entries(
    ...
    """从导出包和写入计划中收集图片资源路径，供 Relay 登记后供插件下载。"""
...
def stage_asset_file(asset_id: str, source: Path, package_dir: Path) -> Path:
    """Copy an image into the package-local .tmp staging area allowed by Relay."""
...
def _iter_nested_prefab_packages(package: dict[str, Any]):
    """深度遍历 nestedPrefabPackages，避免子 Prefab 图片漏传给 Relay。"""
...
def save_screenshot_if_present(result_payload: dict[str, Any], result_path: Path) -> None:
    """保存 Relay 插件回传的截图，并从 JSON 中移除大体积 base64。"""
    ...
    screenshot_dir = result_path.parent / "relay_screenshots"
...
def run_health(args: argparse.Namespace) -> int:
    """只检查 figmaRelay 本地 companion。"""

    relay_url = args.relay_url.rstrip("/")
    payload = ensure_relay(relay_url, startup_timeout=args.startup_timeout)
    print(json.dumps({
        "status": "ok",
        "relayUrl": relay_url,
        "health": payload,
...
def run_export(args: argparse.Namespace) -> int:
    ...
    if session_id:
        ...
        job.setdefault("target", {})["sessionId"] = session_id
    if file_key:
        job["fileKey"] = file_key
        job.setdefault("target", {})["fileKey"] = file_key
    if args.job_output:
    ...
    ensure_relay(relay_url, startup_timeout=args.startup_timeout)
    request_id = str(uuid.uuid4())
    result_payload = relay_submit_job(
        job,
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Prefab To Figma CLI wrapper")
    ...
    parser.add_argument("--result", type=Path, default=DEFAULT_RESULT_PATH,
                        help="prefab_to_figma_relay_result.json 输出路径")
    ...
    parser.add_argument("--session-id", default="", help="Target Figma session")
    parser.add_argument("--file-key", default="", help="Target Figma file")
    ...
    parser.add_argument("--startup-timeout", type=float, default=10.0, help="兼容参数；CLI wrapper 会忽略")
    parser.add_argument("--health", action="store_true", help="只检查 figmaRelay 本地 companion")
